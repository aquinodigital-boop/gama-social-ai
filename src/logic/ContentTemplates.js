        ...
        return {
            ...
            script: [
                ...
                },
                {
                    ...
                    text: isGeneric
                        ? `A Gama tem opções de ${productName} que trabalhamos para sua loja.`
                ...
                {
                    time: "8-15s",
                    visual: "Logística & Gama",
                    text: `Abasteça seu estoque com entrega rápida em todo o ${region}.`,
                    visual_prompt: `(Motion Graphics) Caminhão baú branco com logo da Gama em movimento rápido numa avenida do ABC.`
                },
                {
                    time: "15-20s",
                    visual: "Chamada para Ação",
                    text: "Não perca venda. Clique no link e peça agora!",
                    visual_prompt: `(Texto 3D) Logo da Gama Distribuidora sobre fundo azul corporativo, com botão 'ZAP' pulsando.`
                }
            ],

            // ASSETS (For Editors/Designers)
            assets: {
                narration_text: `"${expertHook}" Aqui na Gama você encontra ${productName} com o melhor preço e entrega rápida no ${region}. Não deixe seu cliente na mão. Link na bio!`,
                ...
                video_prompts: [
                    ...
                    `[Logística] Time lapse de empilhadeira carregando paletes da Gama.`
                ]
            },

            caption: `📦 **${isGeneric ? 'Renove seu Estoque' : 'Item Essencial'}**\n\n${expertHook}\n\n${isGeneric ? `Sua seção de ${productName} precisa estar completa.` : `Garanta ${productName} agora mesmo.`}\n\nA Gama entrega no ${region} com a agilidade que você precisa.\n\n📍 ${categoryContext.keywords.join(" • ")}\n\n#GamaDistribuidora #B2B #${adapterInput.category.replace(/\s/g, '')} #Lojista`
        };
    },
    ...
    generateInstitutionalScript: (themeId) => {
        ...
        return {
            ...
            script: [
                ...
                { time: "5-12s", visual: "Solução Gama", text: script.body, visual_prompt: script.visual_cue },
                { time: "12-15s", visual: "CTA", text: script.cta, visual_prompt: "Equipe de vendas da Gama reunida sorrindo, uniformizados." }
            ],

            assets: {
                narration_text: `${script.hook} ${script.body} ${script.cta}`,
                image_prompts: [
                    `Caminhão de entrega Gama Distribuidora em estrada ensolarada.`,
